...
class main:
    def __init__(self):
        ...
        for i in range(12, 15):
            if i == 14:
                ...
                b = Button(f, text=nums[i], font="Lucida 16 bold", bg="black", fg="orange",command=self.solve_3x3)
                b.pack(side=LEFT, padx=7, pady=7)
                b.bind("<Button-1>", self.click)
            else:
                b = Button(f, text=nums[i], font="Lucida 16 bold", bg="black", fg="white")
    ...
    def solve_3x3(self):
        self.sc_value.set("")
        self.screen.update()
        A = np.array([[numsl[0], numsl[1], numsl[2]], [numsl[3], numsl[4], numsl[5]], [numsl[6], numsl[7], numsl[8]]])
        x = linalg.det(A)
        win = Toplevel()
        win.title("Answer")
