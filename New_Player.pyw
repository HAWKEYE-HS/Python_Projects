...
class MusicPlayer(Frame):
    ...
    def create_Widgets(self):
        ...
        img9=Image.open("Cutter.png")
        img9=img9.resize((50,50),Image.ANTIALIAS)
        self.cutter=ImageTk.PhotoImage(img9)
        f = Frame(self)
        ...
        self.cut=Button(f,command=self.cut,image=self.cutter)
        self.cut.pack(side=LEFT,padx=5,pady=10)
        f.pack()
    ...
    def Stop(self):
        ...
        if self.player.music.get_busy():
            ...
            start.append(self.slider_val.get())
    ...
    def cut(self):
        ...
        s=int(start[len(start)-2])*1000
        e=int(start[len(start)-1])*1000
        audioProcessed=audio[s:e]
...
def New_Window():
    try:
        window = Toplevel()
        file = text.get()
        a = os.path.basename(file).split(".")[0].upper()
        window.title(a)
        l1 = Label(window, text=a, font="Georgia 14", fg="green")
        l1.pack()
...
Button(root, text="Play", bd=4, relief=RAISED, bg="green", command=New_Window, font="Butterfly 12", fg="white").pack(
    pady=10)
l = Label(root, textvariable=update, relief=GROOVE, anchor="w", fg="green")
